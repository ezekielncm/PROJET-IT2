...
class ProprieteBDD extends Propriete{
    ...
    public function getAlldataforpropriete($limit,$ofset){
        ...
        WHERE p.etat = 'libre' AND p.id_agent <>0
...
public function getNbProprieteByBailleurId($id_bailleur) {
  ...
  $data = $stmt->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public function getId_bailleur($id_propriete){
        $sql="SELECT id_bailleur FROM propriete WHERE id_propiete= :id_propriete";
        $stmt=$this->pdo->prepare($sql);
        $stmt->bindValue(':id_propriete',$id_propriete,PDO::PARAM_INT);
        $stmt->execute();
        $data=$stmt->fetch(\PDO::FETCH_ASSOC);
        return $data['id_bailleur'];
    }
